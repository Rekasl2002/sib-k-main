...
abstract class BaseConnection implements ConnectionInterface
{
    ...
    public function newQuery(): BaseBuilder
    {
        ...
        // Untuk menenangkan static analyzer (Intelephense)
        if (method_exists($builder, 'from')) {
            $builder = $builder->from([], true);
        }
    ...
    // Kelas konkret punya getOriginalQuery()
    $sql = $sql->getOriginalQuery();
    } elseif ($sql instanceof QueryInterface) {
        // Fallback aman untuk interface yang tak mendeklarasikan getOriginalQuery()
        $sql = $sql->getQuery();
    }
    ...
    {
        if (is_array($str)) {
            return array_map(fn($v) => $this->escape($v), $str);
        }
    ...
    public function tableExists(string $tableName, bool $cached = true): bool
    {
        ...
        /** @var BaseResult $res */
        $res = $this->query($sql);
        $tableExists = $res->getResultArray() !== [];
