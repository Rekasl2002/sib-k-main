<!-- app/Views/student/violation_submissions/index.php -->
...
if (!function_exists('sibk_fmt_dt')) {
    function sibk_fmt_dt($dt): string
    {
        if (!$dt) return '-';
        try {
            // Jika $dt sudah Time object atau string
            $t = $dt instanceof Time ? $dt : Time::parse((string) $dt);
            // Pakai format yang netral (tanpa tergantung locale server)
...
/**
 * Build label "Terlapor" secara rapi:
 * - Kalau subject_student_id ada: "Nama • Kelas • NISN xxx" (bagian kosong di-skip)
 * - Kalau bukan siswa terdaftar: subject_other_name
 */
if (!function_exists('sibk_subject_label')) {
    function sibk_subject_label(array $r): string
    {
        if (!empty($r['subject_student_id'])) {
            $parts = [];

            $name  = trim((string) ($r['subject_student_name'] ?? ''));
            $class = trim((string) ($r['subject_student_class'] ?? ''));
            $NISN   = trim((string) ($r['subject_student_nisn'] ?? ''));
...
<div class="d-flex align-items-center justify-content-between mb-3">
  <div>
    ...
    <small class="text-muted">Laporkan pelanggaran yang kamu lihat/ketahui. Data akan ditinjau petugas BK.</small>
  </div>
  <a href="<?= base_url('student/violation-submissions/create') ?>" class="btn btn-primary">
    + Buat Pengaduan
  </a>
</div>
            ...
            <tr>
              ...
              <td>
                <a class="btn btn-sm btn-outline-primary"
                   href="<?= base_url('student/violation-submissions/show/' . $id) ?>">
                  Detail
                </a>

                <?php if ($isEditable): ?>
                  <a class="btn btn-sm btn-outline-secondary"
                     href="<?= base_url('student/violation-submissions/edit/' . $id) ?>">
                    Edit
                  </a>

                  <form action="<?= base_url('student/violation-submissions/delete/' . $id) ?>"
                        method="post"
                        class="d-inline"
