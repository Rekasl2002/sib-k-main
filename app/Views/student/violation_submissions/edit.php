<!-- app/Views/student/violation_submissions/edit.php -->
...
<?php
// Helper form untuk old() (umumnya sudah tersedia, tapi ini bikin lebih aman)
...
// Disabled attribute saat terkunci
...
<div class="card">
  <div class="card-body">
    <form action="<?= base_url('student/violation-submissions/update/' . ($row['id'] ?? 0)) ?>"
          method="post"
      ...
      <div class="row">
        <div class="col-md-6 mb-3">
          ...
          <!-- Hidden mirror (agar saat select di-disable, tetap bisa kirim nilai kosong/terkini) -->
          <input type="hidden"
        ...
        <div class="col-md-6 mb-3">
          ...
          <!-- Hidden mirror (agar saat input di-disable, tetap bisa kirim nilai kosong/terkini) -->
          <input type="hidden"
        ...
        <?php if (!empty($evidence)): ?>
          <div class="col-12 mb-3">
            ...
            <small class="text-muted">
              Mencentang “hapus” akan melepas lampiran dari pengaduan. (Audit tetap aman jika sistem kamu menyimpan riwayat.)
            </small>
          </div>
        <?php endif; ?>
      ...
      <div class="d-flex gap-2">
        <a href="<?= base_url('student/violation-submissions/show/' . ($row['id'] ?? 0)) ?>" class="btn btn-light">Batal</a>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function applyLock() {
    ...
    // Prioritas: kalau student dipilih, "other" dikosongkan & dikunci
    if (hasStudent) {
      other.value = '';
    ...
    // Kalau other diisi, dropdown student dikosongkan & dikunci
    if (hasOther) {
      sel.value = '';
    ...
    // Kalau kosong dua-duanya, buka keduanya
    sel.removeAttribute('disabled');
    other.removeAttribute('disabled');
    hidSel.value = '';
    hidOther.value = '';
  ...
  // Initial lock (penting untuk halaman edit)
  applyLock();
