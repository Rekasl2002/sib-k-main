<?php
/**
 * File Path: app/Views/counselor/reports/partials/sections_pdf.php
 *
 * Guru BK • PDF Laporan multi-fitur (server-side, Dompdf)
 * - $sections: array of ['title','columns','rows']
 * - $reportTitle, $schoolName, $periodLabel, $generatedAt, $scopeLabel
 */

$sections    = $sections    ?? [];
$reportTitle = $reportTitle ?? 'Laporan BK (Guru BK)';
  ...
  <?php else: ?>
    <?php foreach ($sections as $sec): ?>
      ...
      <div class="section">
        ...
        <table>
          <thead>
            <tr><?php foreach ($cols as $c): ?><th><?= esc((string) $c) ?></th><?php endforeach; ?></tr>
          </thead>
          <tbody>
            <?php if (!empty($rows)): ?>
