<?php
/**
 * File Path: app/Views/counselor/sanctions/edit.php
 * Edit Sanksi (Counselor/Koordinator)
 */
...
// Util kecil untuk decode JSON dokumen
...
// Fallback untuk type_options bila belum disuplai controller
    ...
    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
      ...
      <div class="page-title-right">
        <ol class="breadcrumb m-0">
          ...
          <li class="breadcrumb-item active">Edit Sanksi</li>
        </ol>
      </div>
    </div>
  </div>
...
<div class="row">
  ...
  <div class="col-lg-8">
    <div class="card">
      ...
      <div class="card-body">
        <form action="<?= base_url('counselor/sanctions/update/'.$sanction['id']) ?>"
              method="post" enctype="multipart/form-data" id="sanctionEditForm">
          <?= csrf_field() ?>
          <input type="hidden" name="violation_id" value="<?= (int)$sanction['violation_id'] ?>">
            ...
            <div class="col-md-3">
              ...
              <select name="status" id="statusSelect" class="form-select" required>
                <?php foreach ($statusOptions as $st): ?>
                  <option value="<?= $st ?>" <?= ($sanction['status'] ?? '') === $st ? 'selected' : '' ?>><?= $st ?></option>
                <?php endforeach; ?>
              </select>
            </div>
          </div>
          ...
          <div class="mt-3">
            ...
            <?php if (!empty($sanction['verified_by'])): ?>
              <small class="text-muted d-block mt-1">
                Sudah diverifikasi sebelumnya pada <?= esc(!empty($sanction['verified_at']) ? date('d M Y H:i', strtotime($sanction['verified_at'])) : '-') ?>
              </small>
            <?php endif; ?>
          </div>
          <?php endif; ?>
                ...
                <li class="mb-2 d-flex align-items-center justify-content-between">
                  ...
                  <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="remove_docs[]"
                           id="rm<?= $i ?>" value="<?= esc($path) ?>">
                    <label class="form-check-label small" for="rm<?= $i ?>">hapus</label>
                  </div>
                </li>
              <?php endforeach; ?>
            </ul>
          ...
          <div class="d-grid d-sm-flex gap-2 mt-4">
            ...
            <a href="<?= base_url('counselor/cases/detail/'.$sanction['violation_id']) ?>" class="btn btn-secondary">
...
(function(){
  ...
  function updateDuration(){
    ...
    if (e >= s && diff >= 0) {
      ...
      box.classList.remove('alert-danger'); box.style.display = 'block';
    } else {
  ...
  // Jika status "Selesai" tapi belum ada completed_date, isi otomatis hari ini pada submit
