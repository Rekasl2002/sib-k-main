<!-- app/Views/parent/child/staff.php -->
...
if (!function_exists('phone_to_wa')) {
    function phone_to_wa(?string $phone): ?string
    {
        ...
        // Normalisasi Indonesia: 0xxx -> 62xxx, 8xxx -> 628xxx
...
function staff_card(string $title, ?array $u, string $studentName, string $classLabel)
{
    ...
    $waText = "Halo, saya orang tua dari {$studentName} (kelas {$classLabel}). Saya ingin bertanya/koordinasi.";
        ...
        <div class="card-body">
            ...
            <?php if ($name): ?>
                ...
                <div class="small text-muted">
                    Tips: Saat menghubungi, sebutkan nama anak dan kelas.
                </div>
            <?php endif; ?>
        </div>
    </div>
...
<div class="card shadow-sm mb-3">
    <div class="card-body">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
            ...
            <!-- Dropdown pindah anak (opsional) -->
            <?php if (!empty($siblings) && count($siblings) > 1): ?>
                <div class="d-flex align-items-center gap-2">
                    <div class="text-muted small">Lihat anak lain:</div>
                    <select class="form-select form-select-sm" style="min-width:220px"
                            onchange="if(this.value) location.href=this.value;">
                        <?php foreach ($siblings as $s): ?>
                            <?php
                                $sid = (int)($s['id'] ?? 0);
                                $label = trim(($s['full_name'] ?? '—') . ' • ' . ($s['class_name'] ?? '—'));
                                $url = $sid ? route_to('parent.children.staff', $sid) : '';
                            ?>
                            <option value="<?= esc($url, 'attr') ?>" <?= $sid === (int)($student['id'] ?? 0) ? 'selected' : '' ?>>
                                <?= esc($label) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
        ...
        <div class="mt-3">
            <a class="btn btn-outline-secondary btn-sm" href="<?= route_to('parent.dashboard') ?>">
