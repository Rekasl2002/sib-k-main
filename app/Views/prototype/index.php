...
<style>
  ...
  .prototype-page .flow-step {
    border-left: 3px solid rgba(31, 111, 84, .35);
    padding-left: 14px;
  }
  ...
  .prototype-page .proto-card.is-locked {
    opacity: .62;
  }
</style>
<?= $this->endSection() ?>
...
$startUrl = (string) ($startUrl ?? ($firstFeature['url'] ?? 'prototype'));
?>
...
<div class="prototype-page">
  <nav aria-label="breadcrumb" class="mb-3 d-flex justify-content-end">
    <ol class="breadcrumb mb-0">
      ...
      <li class="breadcrumb-item active" aria-current="page">Prototipe Skripsi</li>
    </ol>
  </nav>

  <div class="row align-items-center mb-4">
    <div class="col-lg-8">
      <h2 class="page-title mb-2">Prototipe Fitur Skripsi</h2>
      <p class="text-muted mb-0">
        Ruang simulasi untuk lima fitur pengembangan SIB-K. Semua contoh di halaman ini memakai data dummy dan tidak menulis ke database utama.
      </p>
    </div>
    <div class="col-lg-4 text-lg-end mt-3 mt-lg-0">
      <a href="<?= base_url($startUrl) ?>" class="btn btn-primary">
        <i class="mdi mdi-play-circle-outline me-1"></i> Mulai Prototipe
      </a>
    </div>
  </div>
  ...
  <div class="role-switch p-3 mb-4">
    <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3">
      <div>
        ...
        <small class="text-muted">Fitur prototipe, tombol aksi, dan data contoh mengikuti kebutuhan role yang dipilih.</small>
      </div>
  ...
  <div class="row g-3">
    <?php foreach ($features as $key => $item): ?>
      ...
      <div class="col-lg-6 col-xl-4">
        ...
        <?php else: ?>
          <div class="proto-link" aria-disabled="true">
        <?php endif; ?>
          <div class="card proto-card h-100 <?= $isLocked ? 'is-locked' : '' ?>">
            <div class="card-body">
              ...
              <p class="text-muted mb-3"><?= esc($item['outcome'] ?? '') ?></p>
              <span class="btn btn-sm <?= $isLocked ? 'btn-outline-secondary disabled' : ($isTried ? 'btn-outline-success' : 'btn-outline-primary') ?>">
                ...
                <?php else: ?>
                  Buka Prototipe <i class="mdi mdi-arrow-right ms-1"></i>
                <?php endif; ?>
              </span>
            </div>
          </div>
  ...
  <div class="card proto-card mt-4">
    <div class="card-body">
      <div class="row g-3 align-items-start">
        <div class="col-lg-4">
          <h5 class="mb-2">Skenario Demo</h5>
          <p class="text-muted mb-0">Alur ini disiapkan untuk validasi awal kebermanfaatan bersama calon pengguna sebelum data asli dan aturan produksi diaktifkan.</p>
        </div>
        <div class="col-lg-8">
          <div class="row g-3">
            <div class="col-md-4">
              <div class="flow-step">
                <div class="fw-semibold">1. Simulasi input</div>
                <small class="text-muted">Pengguna mencoba mengirim data contoh.</small>
              </div>
            </div>
            <div class="col-md-4">
              <div class="flow-step">
                <div class="fw-semibold">2. Simulasi proses</div>
                <small class="text-muted">Petugas melihat status, prioritas, dan tindak lanjut.</small>
              </div>
            </div>
            <div class="col-md-4">
              <div class="flow-step">
                <div class="fw-semibold">3. Simulasi hasil</div>
                <small class="text-muted">Ringkasan luaran bisa dinilai oleh pengguna.</small>
              </div>
            </div>
          </div>
        </div>
