...
/**
 * File Path: app/Views/koordinator/cases/index.php
 *
 * Koordinator • Cases Index View
 * Tampilan daftar kasus pelanggaran siswa dengan filter, statistik, dan aksi
 *
 * Catatan:
 * - Semua action/URL mengarah ke prefix /koordinator
 * - Mendukung filter repeat offender via checkbox (is_repeat_offender=1)
 * - Kompatibel dengan $stats berbentuk $stats['overall'][key] atau $stats[key]
 * - Pagination hanya menggunakan View (DataTables) -> tidak memakai $pager->links()
                            ...
                            <tbody>
                                <?php foreach ($violationRows as $violation): ?>
                                    <?php
                                        ...
                                        $orderDate = trim($vDate . ' ' . ($vTime ?: '00:00:00')); // untuk sort DataTables
                                    ?>
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <div class="btn-group" role="group">
                                                <a href="<?= base_url('koordinator/cases/detail/' . (int)($violation['id'] ?? 0)) ?>"
                                                   class="btn btn-sm btn-info"
                                                   title="Detail">
                                                    <i class="mdi mdi-eye"></i>
                                                </a>

                                                <!-- EDIT selalu boleh (U) untuk Koordinator -->
                                                <a href="<?= base_url('koordinator/cases/edit/' . (int)($violation['id'] ?? 0)) ?>"
                                                   class="btn btn-sm btn-warning"
                                                   title="Ubah">
                                                    <i class="mdi mdi-pencil"></i>
                                                </a>

                                                <!-- HAPUS hanya jika manage_violations dicentang -->
                                                <?php if ($canManageViolations): ?>
                                                    <button type="button"
                                                            class="btn btn-sm btn-danger"
                                                            onclick="deleteViolation(<?= (int)($violation['id'] ?? 0) ?>)"
                                                            title="Hapus">
...
<script>
    ...
    // Handler untuk tombol .btn-delete jika ada form inline (fallback)
