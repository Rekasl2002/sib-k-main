<?php
/**
 * File Path: app/Views/koordinator/sanctions/show.php
 * Detail Sanksi (Koordinator)
 * - UI disamakan dengan counselor/sanctions/show.php
 * - Quick action pakai endpoint Koordinator: verify/complete/acknowledge (bukan update)
 */
...
// Koordinator area: biasanya selalu boleh edit/delete, tapi tetap aman kalau helper role tidak ada
$canEdit   = function_exists('is_koordinator') ? (bool) is_koordinator() : true;
$canDelete = $canEdit;
?>
        ...
        <?php if (!empty($docs)): ?>
          <ul class="list-unstyled mb-0">
            <?php foreach ($docs as $doc):
              $path = is_string($doc) ? $doc : ($doc['path'] ?? '');
              if (!$path) continue;
              $href = base_url($path);
    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="d-grid gap-2">
          <?php if ($canEdit): ?>
            <a href="<?= base_url('koordinator/sanctions/edit/'.(int)($sanction['id'] ?? 0)) ?>" class="btn btn-primary w-100">
              <i class="mdi mdi-pencil me-1"></i>Edit Sanksi
            </a>
          <?php endif; ?>

          <?php if ((function_exists('is_koordinator') && is_koordinator()) && empty($sanction['verified_by'])): ?>
            <form action="<?= base_url('koordinator/sanctions/verify/'.(int)($sanction['id'] ?? 0)) ?>" method="post" class="d-grid">
              <?= csrf_field() ?>
          ...
          <?php if (($sanction['status'] ?? '') !== 'Selesai'): ?>
            <form action="<?= base_url('koordinator/sanctions/complete/'.(int)($sanction['id'] ?? 0)) ?>" method="post" class="d-grid">
              <?= csrf_field() ?>
          ...
          <?php if (empty($sanction['parent_acknowledged'])): ?>
            <form action="<?= base_url('koordinator/sanctions/acknowledge/'.(int)($sanction['id'] ?? 0)) ?>" method="post" class="d-grid">
              <?= csrf_field() ?>
              <input type="hidden" name="parent_acknowledged_at" value="<?= date('Y-m-d\TH:i') ?>">
          ...
          <?php if ($canDelete): ?>
            <form action="<?= base_url('koordinator/sanctions/delete/'.(int)($sanction['id'] ?? 0)) ?>" method="post"
                  class="d-grid"
          ...
          <a href="<?= base_url('koordinator/cases/detail/'.(int)($sanction['violation_id'] ?? 0)) ?>" class="btn btn-secondary w-100">
