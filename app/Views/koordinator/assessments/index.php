<?php
/**
 * Daftar Asesmen — Koordinator BK
 * Akses & tampilan disamakan dengan Guru BK (counselor/assessments/index),
 * dengan file view milik Koordinator sendiri. Beda hanya pola URL rute
 * Koordinator (show/edit/delete/duplicate/publish memakai verb/{id}).
 */
?>
...
$prefix = 'koordinator/assessments';
                ...
                <tr>
                  ...
                  <td>
                    <div class="fw-semibold text-dark">
                      <a href="<?= site_url($prefix . '/show/' . $aid) ?>" class="text-dark text-decoration-none"><?= esc($a['title'] ?? '-') ?></a>
                    </div>
                  ...
                  <td>
                    <div class="d-flex flex-column gap-1 action-stack mx-auto">
                      <a href="<?= site_url($prefix . '/show/' . $aid) ?>" class="btn btn-sm btn-info text-start" data-bs-toggle="tooltip" title="Lihat detail asesmen">
                      ...
                      <a href="<?= site_url($prefix . '/edit/' . $aid) ?>" class="btn btn-sm btn-warning text-start" data-bs-toggle="tooltip" title="Ubah asesmen">
                        <i class="mdi mdi-pencil me-1"></i> Edit
                      </a>
                      <?php if (! $published): ?>
                        <form method="post" action="<?= site_url($prefix . '/publish/' . $aid) ?>" onsubmit="return confirm('Publikasikan asesmen ini?');">
                          <?= csrf_field() ?>
                      ...
                      <?php else: ?>
                        <form method="post" action="<?= site_url($prefix . '/unpublish/' . $aid) ?>" onsubmit="return confirm('Batalkan publikasi asesmen ini?');">
                          <?= csrf_field() ?>
                      ...
                      <?php endif; ?>
                      <form method="post" action="<?= site_url($prefix . '/duplicate/' . $aid) ?>" onsubmit="return confirm('Salin/duplikat asesmen ini?');">
                        <?= csrf_field() ?>
                      ...
                      <form method="post" action="<?= site_url($prefix . '/delete/' . $aid) ?>" onsubmit="return confirm('Hapus asesmen ini? Semua data terkait akan ikut terhapus.');">
                        <?= csrf_field() ?>
