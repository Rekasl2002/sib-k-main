  ...
  <div class="col-12">
    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
      ...
      <div class="d-flex gap-2 flex-wrap mt-2 mt-sm-0 align-items-center">
        ...
        <a href="<?= base_url('koordinator/assessments/edit/'.$aid) ?>" class="btn btn-outline-secondary"><i class="mdi mdi-pencil me-1"></i>Edit</a>
      </div>
    </div>
  </div>
</div>
  ...
  <div class="col-lg-8">
    ...
    <div class="card border-0 shadow-sm mb-4">
      <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
        ...
        <a href="<?= base_url('koordinator/assessments/'.$aid.'/questions') ?>" class="btn btn-sm btn-outline-primary">
    ...
    <div class="card border-0 shadow-sm mb-4">
      ...
      <div class="card-body">
        <div class="d-grid gap-2">
          <a href="<?= base_url('koordinator/assessments/'.$aid.'/assign') ?>" class="btn btn-primary">
            <i class="fas fa-user-plus me-2"></i>Tugaskan
          </a>

          <?php if (empty($assessment['is_published'])): ?>
            <form method="post" action="<?= base_url('koordinator/assessments/publish/'.$aid) ?>" onsubmit="return confirm('Publikasikan asesmen ini?')">
              <?= csrf_field() ?>
              <button type="submit" class="btn btn-success w-100"><i class="fas fa-share me-2"></i>Publikasi</button>
            </form>
          <?php else: ?>
            <form method="post" action="<?= base_url('koordinator/assessments/unpublish/'.$aid) ?>" onsubmit="return confirm('Batalkan publikasi asesmen ini?')">
              <?= csrf_field() ?>
              <button type="submit" class="btn btn-warning w-100"><i class="fas fa-times-circle me-2"></i>Batalkan Publikasi</button>
            </form>
          <?php endif; ?>

          <form method="post" action="<?= base_url('koordinator/assessments/duplicate/'.$aid) ?>">
            <?= csrf_field() ?>
            <button type="submit" class="btn btn-outline-secondary w-100"><i class="fas fa-copy me-2"></i>Duplikat</button>
          </form>

          <a href="<?= base_url('koordinator/assessments/'.$aid.'/results') ?>" class="btn btn-outline-info">
            <i class="fas fa-chart-bar me-2"></i>Lihat Hasil
          </a>

          <!-- Opsional: sinkronkan penugasan ke results (membuat baris Assigned) -->
          <form method="post" action="<?= base_url('koordinator/assessments/'.$aid.'/assign/sync') ?>" onsubmit="return confirm('Sinkronkan penugasan ke daftar hasil?')">
            <?= csrf_field() ?>
          ...
          <form method="post" action="<?= base_url('koordinator/assessments/delete/'.$aid) ?>" onsubmit="return confirm('Hapus asesmen ini beserta data terkait?')">
            <?= csrf_field() ?>
