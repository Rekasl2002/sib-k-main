<?php
/**
 * File Path: app/Views/koordinator/career/student_choices.php
 * Pilihan Karier dan Studi Lanjut Siswa (Koordinator BK)
 *
 * Tampilan diseragamkan dengan Manajemen Pengguna/Siswa: page-title-box,
 * kartu Filter/Saring Data, kartu daftar + DataTables. Setiap baris punya
 * tombol "Detail" untuk melihat detail karier / perguruan tinggi.
 */
...
$baseUrl = site_url('koordinator/career-info/student-choices');
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="class_id" class="form-select">
                                ...
                                <?php foreach ($classes as $cls): ?>
                                    <option value="<?= (int) $cls['id'] ?>" <?= (string)($filters['class_id'] ?? '') === (string)$cls['id'] ? 'selected' : '' ?>>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="sort" class="form-select">
                                ...
                                <?php if ($activeTab === 'careers'): ?>
                                    <option value="career" <?= ($filters['sort'] ?? '') === 'career' ? 'selected' : '' ?>>Nama Karier</option>
                                <?php else: ?>
                                    <option value="name" <?= ($filters['sort'] ?? '') === 'name' ? 'selected' : '' ?>>Nama Perguruan Tinggi</option>
                                <?php endif; ?>
                                <option value="latest" <?= ($filters['sort'] ?? '') === 'latest' ? 'selected' : '' ?>>Terbaru Disimpan</option>
                            </select>
                        </div>
                    </div>
                                ...
                                <?php foreach (($careerChoices ?? []) as $row): ?>
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <a href="<?= route_to('koordinator.career.show', (int)($row['career_id'] ?? 0)) ?>" class="btn btn-sm btn-info" title="Detail Karier" data-bs-toggle="tooltip"><i class="mdi mdi-eye"></i></a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                                </tbody>
                            ...
                            <table id="choicesTable" class="table table-hover table-bordered nowrap w-100">
                                ...
                                <?php foreach (($universityChoices ?? []) as $row): ?>
                                    <tr>
                                        ...
                                        <td><?= esc($row['saved_at'] ?? '-') ?></td>
                                        <td class="text-center">
                                            <a href="<?= route_to('koordinator.university.show', (int)($row['university_id'] ?? 0)) ?>" class="btn btn-sm btn-info" title="Detail Perguruan Tinggi" data-bs-toggle="tooltip"><i class="mdi mdi-eye"></i></a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                                </tbody>
