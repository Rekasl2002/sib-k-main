...
?>
<div class="row"><div class="col-12"><div class="page-title-box d-sm-flex align-items-center justify-content-between"><div><h4 class="mb-sm-0"><?= esc($row['title'] ?? 'Penugasan') ?></h4><p class="text-muted mb-0"><?= esc($row['assignment_type'] ?? '-') ?> - <?= esc($row['status'] ?? '-') ?></p></div><div class="d-flex gap-2 align-items-center flex-wrap justify-content-end"><ol class="breadcrumb m-0 me-2"><li class="breadcrumb-item"><a href="<?= base_url('koordinator/dashboard') ?>">Koordinator</a></li><li class="breadcrumb-item"><a href="<?= site_url($routePrefix) ?>">Penugasan</a></li><li class="breadcrumb-item active">Detail</li></ol><a href="<?= site_url($routePrefix) ?>" class="btn btn-outline-secondary">Kembali</a><?php if (! empty($canManage)): ?><a href="<?= site_url($routePrefix . '/edit/' . (int) $row['id']) ?>" class="btn btn-primary">Edit</a><?php endif; ?></div></div></div></div>
<div class="row">
  <div class="col-lg-8">
